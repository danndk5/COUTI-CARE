...
// ── Helper GPS overlay (sama seperti FormScreen) ─────────────────────────────
const decimalToDMS = (d, p, n) => {
  const dir = d >= 0 ? p : n, abs = Math.abs(d);
  const deg = Math.floor(abs), mf = (abs - deg) * 60;
  const min = Math.floor(mf), sec = Math.round((mf - min) * 60);
...
// ── CameraCapture (inline, sama seperti FormScreen) ──────────────────────────
const CameraCapture = ({ label, onFile, fotoUrl, onRemove, errorFoto }) => {
  ...
  const handleClick = async () => {
    ...
    catch (e) { setCapState("idle"); setPermErr(e.message==="camera" ? "Izin kamera diperlukan. Aktifkan di pengaturan browser." : "Izin lokasi (GPS) diperlukan."); }
  };
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ border:`2px dashed ${borderColor}`, borderRadius:10, padding:"14px 12px", textAlign:"center", background: errorFoto?theme.dangerLight:"transparent" }}>
        {fotoUrl ? (
          ...
          <>
            <div style={{fontSize:12,color:errorFoto?theme.danger:theme.textMuted,marginBottom:8}}>{label}</div>
            ...
            <Btn onClick={handleClick} variant="outline" style={{padding:"7px",fontSize:12}} disabled={isWorking}>
  ...
  return (
    <div style={{ background: theme.surface, border:`1px solid ${theme.border}`, borderRadius:14, padding:16, marginBottom:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <div style={{ fontWeight:700, fontSize:14, color:theme.primary }}>Temuan #{idx+1}</div>
        {showRemove && (
          <div onClick={() => onRemove(idx)} style={{ cursor:"pointer", fontSize:12, color:theme.danger, fontWeight:600 }}>
            Hapus
          </div>
        )}
      </div>
      <Input label="Judul / Objek yang Diperiksa" placeholder="Contoh: Ban, Lampu, Angin..." value={item.judul} onChange={set("judul")} />
      <div style={{ marginBottom:12 }}>
        <div style={{ fontSize:13, fontWeight:600, color:theme.textSub, marginBottom:6 }}>Keterangan Temuan</div>
        <textarea
          placeholder="Tuliskan temuan secara detail..."
          value={item.keterangan}
          onChange={(e) => set("keterangan")(e.target.value)}
          style={{ width:"100%", padding:"10px 12px", borderRadius:10, border:`1.5px solid ${theme.border}`, background:theme.surfaceAlt, color:theme.text, fontSize:13, fontFamily:"'DM Sans',sans-serif", resize:"none", minHeight:80, boxSizing:"border-box", outline:"none" }}
        />
      </div>
      <CameraCapture
...
const P1FormScreen = ({ onBack, onNav }) => {
  ...
  // Temuan (dinamis)
  const [temuan, setTemuan] = useState([{ judul:"", keterangan:"", foto:null, errorFoto:false }]);
  ...
  // Auto-fill kendaraan dari nomor polisi
  const handleNopolChange = useCallback((val) => {
    ...
    lookupTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        if(data) {
          setTransportir(data.transportir || ""); setKapasitas(data.kapasitas_mt || "");
          setKompartemen(String(data.jumlah_kompartemen || "")); setKategoriMT(data.kategori_mt || "");
  ...
  const resetAutoFill = () => { setIsAutoFilled(false); setLookupStatus("new"); setKapasitas(""); setKompartemen(""); setTransportir(""); setKategoriMT(""); };

  // Temuan handlers
  const updateTemuan = (idx, key, val) => setTemuan(prev => prev.map((t,i) => i===idx ? {...t,[key]:val} : t));
  const addTemuan = () => setTemuan(prev => [...prev, {judul:"", keterangan:"", foto:null, errorFoto:false}]);
  ...
  const validateStep2 = () => {
    if(!kategoriMT) { alert("Pilih kategori MT!"); return false; }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    const updated = temuan.map(t => {
      const errFoto = !t.foto;
      if(!t.judul.trim() || !t.keterangan.trim() || errFoto) valid = false;
      return {...t, errorFoto: errFoto};
    });
    setTemuan(updated);
    if(!valid) { alert("Semua temuan wajib ada judul, keterangan, dan foto!"); return; }
    ...
    try {
      ...
      for(const t of temuan) {
        ...
        if(t.foto?.url) {
          await supabase.from("foto_inspeksi_p1").insert([{ inspeksi_id: insp.id, temuan_id: tv.id, url: t.foto.url }]);
        }
      }

      // Upsert kendaraan
      await supabase.from("kendaraan").upsert({
        nomor_polisi: nopol.trim(), transportir: transportir.trim(),
        kapasitas_mt: kapasitas.trim(), jumlah_kompartemen: parseInt(kompartemen)||0,
        kategori_mt: kategoriMT, updated_at: new Date().toISOString(),
  ...
  return (
    <div style={{minHeight:"100vh",background:theme.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:theme.surface,padding:"48px 16px 16px",borderBottom:`1px solid ${theme.border}`,boxShadow:theme.shadow}}>
        ...
        <div style={{fontWeight:800,fontSize:18,color:theme.text,marginBottom:16}}>Pengecekan Cek Random</div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"20px 16px",paddingBottom:90}}>
        ...
        {step===1 && (
          <>
            ...
            <div style={{background:theme.surface,borderRadius:14,padding:16,border:`1px solid ${theme.border}`}}>
              ...
              {lookupStatus==="loading" && <div style={{fontSize:12,color:theme.textMuted,marginBottom:10}}>🔍 Mencari data...</div>}
              ...
              {lookupStatus==="new"     && <div style={{fontSize:12,color:"#D97706",fontWeight:600,marginBottom:10}}>🆕 Kendaraan baru — isi manual, tersimpan otomatis</div>}
              ...
              {isAutoFilled && (
                <div style={{fontSize:11,color:theme.textMuted,marginTop:-8,marginBottom:12}}>
                  Terisi otomatis. <span onClick={resetAutoFill} style={{color:theme.primary,cursor:"pointer",textDecoration:"underline"}}>Edit manual</span>
                </div>
              )}
            </div>
          </>
        ...
        {step===2 && (
          <>
            ...
            <div style={{fontSize:13,color:theme.textMuted,marginBottom:16}}>
              Pilih jenis MT berdasarkan peruntukannya.
            </div>
            {[
              { val:"merah_putih", label:"MT Merah Putih",  desc:"Untuk SPBU",                        emoji:"🔴" },
              ...
              <div key={k.val} onClick={() => setKategoriMT(k.val)} style={{
                ...
                border:`2px solid ${kategoriMT===k.val?theme.primary:theme.border}`,
                background:kategoriMT===k.val?theme.primaryLight:theme.surface,
              }}>
                <div style={{fontWeight:700,fontSize:15,color:kategoriMT===k.val?theme.primary:theme.text}}>
                  {k.emoji} {k.label}
                </div>
        ...
        {step===3 && (
          <>
            ...
            <div style={{fontSize:13,color:theme.textMuted,marginBottom:16}}>
              Isi temuan yang ditemukan. Klik <b>Tambah</b> untuk menambah temuan baru.
            </div>
            {temuan.map((t,i) => (
              <TemuanItem key={i} idx={i} item={t} onChange={updateTemuan} onRemove={removeTemuan} showRemove={temuan.length>1} />
            ))}
            <div onClick={addTemuan} style={{
              display:"flex",alignItems:"center",justifyContent:"center",gap:8,
              padding:"14px",borderRadius:12,border:`2px dashed ${theme.primary}`,
              color:theme.primary,fontWeight:600,fontSize:13,cursor:"pointer",marginBottom:16,
              background:theme.primaryLight,
            }}>
              <Icon name="plus" size={16} color={theme.primary} /> Tambah Pengecekan
            </div>
          </>
        )}
      </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,padding:"12px 16px",background:theme.surface,borderTop:`1px solid ${theme.border}`,display:"flex",gap:10}}>
        ...
        {step===1 && (
          <Btn onClick={()=>{if(validateStep1())setStep(2)}} variant="primary" disabled={submitting}>Lanjut →</Btn>
        )}
        {step===2 && (
          <Btn onClick={()=>{if(validateStep2())setStep(3)}} variant="primary" disabled={submitting}>Lanjut →</Btn>
        )}
        {step===3 && (

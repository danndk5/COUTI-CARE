...
const CameraCapture = ({ label, kategori, onPhotos, allPhotos, errorFoto }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (error) { alert("Foto gagal diupload: " + error.message); return; }
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ border: `2px dashed ${errorFoto ? theme.danger : theme.border}`, borderRadius: 12, padding: "14px 16px", background: errorFoto ? theme.dangerLight : "transparent" }}>
        ...
        <Btn onClick={handleCaptureClick} variant="outline" style={{ padding: "9px", fontSize: 13, width: "100%" }} disabled={capState !== "idle"}>
          {capState === "checking" ? "🔐 Cek izin..." : capState === "processing" ? "⏳ Memproses..." : "📷 Ambil Foto"}
        </Btn>
        {photos.length > 0 && (
          <div style={{ marginTop: 10 }}>
            {photos.map((p) => (
              <div key={p.path} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: theme.primaryLight, borderRadius: 8, marginBottom: 6, fontSize: 12, color: theme.primary }}>
                <span>✓ {p.name}</span>
...
const CheckItemWithFoto = ({ label, status, onStatus, ket, onKet, errorKet, kategori, onPhotos, allPhotos, errorFoto }) => (
  <div style={{ marginBottom: 14, padding: 14, borderRadius: 12, background: theme.surfaceAlt, border: `1px solid ${theme.border}` }}>
    ...
    <div style={{ marginTop: 10 }}>
      <CameraCapture label="Foto dokumentasi" kategori={kategori} onPhotos={onPhotos} allPhotos={allPhotos} errorFoto={errorFoto} />
    </div>
  </div>
);
...
const FormScreen = ({ onBack, onNav }) => {
  ...
  const [gps, setGps] = useState({ status: "", segel: { status: "", ket: "" }, kabel: { status: "", ket: "" } });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (photos.length > 0) {
        ...
        if (fotoErr) { alert("Laporan tersimpan, tapi foto gagal: " + fotoErr.message); onNav("dashboard"); return; }
      }
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px", paddingBottom: 100 }}>
        ...
        {step === 2 && (
          <>
            ...
            <StatusAktifWithFoto label="Status GPS" status={gps.status} onStatus={(v) => setGps((p) => ({ ...p, status: v }))} kategori="gps_status" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.gps_status_foto} />
            ...
            <CheckItemWithFoto label="Segel GPS" status={gps.segel.status} onStatus={setGpsField("segel", "status")} ket={gps.segel.ket} onKet={setGpsField("segel", "ket")} errorKet={errors.gps_segel_ket} kategori="gps_segel" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.gps_segel_foto} />
            <CheckItemWithFoto label="Kabel GPS" status={gps.kabel.status} onStatus={setGpsField("kabel", "status")} ket={gps.kabel.ket} onKet={setGpsField("kabel", "ket")} errorKet={errors.gps_kabel_ket} kategori="gps_kabel" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.gps_kabel_foto} />
          </>
        )}
        ...
        {step === 3 && (
          <>
            ...
            <StatusAktifWithFoto label="Status CCTV Dashcam" status={cctv.dashcam.status} onStatus={setCctvField("dashcam", "status")} kategori="cctv_dashcam_status" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.dashcam_status_foto} />
            ...
            <CheckItemWithFoto label="Segel Sambungan Kabel" status={cctv.dashcam.segel_kabel} onStatus={setCctvField("dashcam", "segel_kabel")} ket={cctv.dashcam.ket_kabel} onKet={setCctvField("dashcam", "ket_kabel")} errorKet={errors.dashcam_kabel_ket} kategori="cctv_dashcam_kabel" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.dashcam_kabel_foto} />
            <SectionLabel style={{ marginTop: 8 }}>CCTV Kanan</SectionLabel>
            <StatusAktifWithFoto label="Status CCTV Kanan" status={cctv.kanan.status} onStatus={setCctvField("kanan", "status")} kategori="cctv_kanan_status" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.kanan_status_foto} />
            ...
            <CheckItemWithFoto label="Segel Sambungan Kabel" status={cctv.kanan.segel_kabel} onStatus={setCctvField("kanan", "segel_kabel")} ket={cctv.kanan.ket_kabel} onKet={setCctvField("kanan", "ket_kabel")} errorKet={errors.kanan_kabel_ket} kategori="cctv_kanan_kabel" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.kanan_kabel_foto} />
            <SectionLabel style={{ marginTop: 8 }}>CCTV Kiri</SectionLabel>
            <StatusAktifWithFoto label="Status CCTV Kiri" status={cctv.kiri.status} onStatus={setCctvField("kiri", "status")} kategori="cctv_kiri_status" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.kiri_status_foto} />
            <CheckItemWithFoto label="Segel Bricket" status={cctv.kiri.segel_bricket} onStatus={setCctvField("kiri", "segel_bricket")} ket={cctv.kiri.ket_bricket} onKet={setCctvField("kiri", "ket_bricket")} errorKet={errors.kiri_bricket_ket} kategori="cctv_kiri_bricket" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.kiri_bricket_foto} />
            <CheckItemWithFoto label="Segel Sambungan Kabel" status={cctv.kiri.segel_kabel} onStatus={setCctvField("kiri", "segel_kabel")} ket={cctv.kiri.ket_kabel} onKet={setCctvField("kiri", "ket_kabel")} errorKet={errors.kiri_kabel_ket} kategori="cctv_kiri_kabel" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.kiri_kabel_foto} />
            <SectionLabel style={{ marginTop: 8 }}>Segel Kotak Sekring</SectionLabel>
            <StatusAktifWithFoto label="Status Segel Kotak Sekring" status={segelKotakSekring} onStatus={setSegelKotakSekring} kategori="segel_kotak_sekring" onPhotos={setPhotos} allPhotos={photos} errorFoto={errors.segel_kotak_foto} />
